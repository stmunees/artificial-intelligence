...
from expectimax import alphabeta
...
def get_empty_around_piece(piece, board):
    ...
    x = int(piece / 10)
    y = piece % 10;
    surroundings = [[x - 1, y], [x + 1, y], [x, y - 1], [x, y + 1], [x - 1, y - 1], [x - 1, y + 1], [x + 1, y - 1],
                    [x + 1, y + 1]]
    result = []
...
def get_legal_moves(player, board):
    ...
    empty_around_opponent = []
    for op in opppieces:
        empty_around_opponent.extend(get_empty_around_piece(op, board))

    for j in empty_around_opponent:
        for i in ownpieces:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        while True:
            print("Your choices: " + str(moves.keys()))
            move = int(input('Enter Choice:'))
            if move in moves.keys():
                board[move] = '@'
                player_score += 1
                num = do_flips('@', move, moves[move], board)
                print(num)
                player_score += num
                comp_score -= num
                print(print_board(board))
            ...
            if game_over(board):  # checks if game is over
                exit()
        computer_moves = get_legal_moves('o', board)
        print("Computer Choices: " + str(computer_moves.keys()))

        max_score, best_move = expectimax_search('o', board, 3)
        b = computer_moves[best_move]
        print("Move chosen:" + str(best_move))
        board[best_move] = 'o';
        comp_score += 1
        num = do_flips('o', best_move, b, board)
        print(num)
        player_score -= num
        comp_score += num
        print(print_board(board))
